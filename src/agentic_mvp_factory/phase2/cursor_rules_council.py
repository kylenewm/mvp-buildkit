...
def run_cursor_rules_council(
    ...
    # 2. Create new run for cursor rules generation
    ...
    with ThreadPoolExecutor(max_workers=len(models)) as executor:
        futures = {
            executor.submit(_generate_cursor_rules_draft, run_id, model, plan_content): model
            for model in models
        }
    ...
    with ThreadPoolExecutor(max_workers=len(models)) as executor:
        futures = {
            executor.submit(_generate_cursor_rules_critique, run_id, model, plan_content, drafts_text): model
            for model in models
        }
